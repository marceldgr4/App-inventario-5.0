...
function getComentarioData() {
  return JSON.stringify({
    data: _getInventoryDataForSheet(getHojasConfig().COMENTARIOS.nombre),
  });
}
...
function agregarComentarioGenerico(sheetName, productoId, comentario) {
  try {
    ...
    if (!ss)
      return JSON.stringify({
        success: false,
        message: "No se pudo acceder a la hoja de cálculo principal.",
      });
    ...
    if (!sheet) {
      ...
      for (let s of sheets) {
        if (s.getName().toLowerCase().replace(/\s/g, "") === target) {
          sheet = s;
          break;
        }
      }
    ...
    if (!sheet)
      return JSON.stringify({
        success: false,
        message: `Hoja origen '${sheetName}' no encontrada.`,
      });

    const data = sheet.getDataRange().getValues();
    if (!data || data.length === 0)
      return JSON.stringify({ success: false, message: "Hoja sin datos." });
    ...
    const rowIndex = data.findIndex(
      (r, idx) => idx > 0 && String(r[idIndex]) === String(productoId)
    );
    if (rowIndex === -1)
      return JSON.stringify({
        success: false,
        message: `Producto ${productoId} no encontrado en ${sheet.getName()}`,
      });
    ...
    const usuario =
      Session && Session.getActiveUser
        ? Session.getActiveUser().getEmail()
        : "Anónimo";
    const fecha = new Date();

    // Guardar en hoja central de comentarios
    guardarComentarioEnSheet(
      productoId,
      producto,
      programa,
      comentario,
...
function guardarComentarioEnSheet(
  productoId,
  producto,
  programa,
  comentario,
  ...
  try {
    ...
    if (!commentSheet)
      throw new Error("Hoja de comentarios no encontrada: " + commentSheetName);
    ...
    const sourceSheetConfig = Object.values(hojasConfig).find((config) =>
      config.nombre.toUpperCase().replace(/\s/g, "").includes(programaKey)
    );
      ...
      if (sourceSheet) {
        ...
        if (idIndex !== -1 && comentariosIndex !== -1) {
          ...
          const rowIndex =
            data
              .slice(1)
              .findIndex((r) => String(r[idIndex]) === String(productoId)) + 2;
          if (rowIndex > 1) {
            // si es > 1, significa que encontró la fila (el índice sería >= 0)
            sourceSheet
              .getRange(rowIndex, comentariosIndex + 1)
              .setValue(comentario);
            Logger.log(
              `Comentario actualizado en la hoja '${sourceSheetConfig.nombre}' para el producto ID ${productoId}.`
            );
          } else {
            Logger.log(
              `WARN: No se encontró el producto con ID ${productoId} en la hoja '${sourceSheetConfig.nombre}'.`
            );
          }
        } else {
          Logger.log(
            `WARN: No se encontró la columna 'Id' o 'Comentarios' en la hoja '${sourceSheetConfig.nombre}'.`
          );
        }
      } else {
        Logger.log(
          `WARN: No se pudo encontrar la hoja de origen '${sourceSheetConfig.nombre}'.`
        );
      }
    } else {
      Logger.log(
        `WARN: No se encontró configuración de hoja para el programa '${programa}'.`
      );
    }

    return JSON.stringify({
      success: true,
      message: "Comentario guardado y actualizado.",
    });
  } catch (e) {
